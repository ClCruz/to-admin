      ...
      <b-container>
        ...
        <b-row>
          <b-table striped="striped"
              ...
              <template slot="StaTipForPagto" slot-scope="data">
                  <span v-if="data.item.StaTipForPagto == 'A'">Ativo</span>
                  <span v-else>Inativo</span>
              </template>

              <template slot="actions" slot-scope="data">
                  <span v-if="!mayI('typeofpt-add')">-</span>
                  <b-button-group size="sm" v-if="mayI('typeofpt-add')">
                      <b-button title="Editar" v-if="mayI('typeofpt-add')" @click.stop="edit(data.item,$event.target)">Editar</b-button>
                  </b-button-group>
              </template>
          </b-table>
...
export default {
  ...
  computed: {
    mayIsee() {
      return this.mayI('typeofpt-viewer', 'typeofpt-add');
    }
  },
  created() {
    this.populateBases();
  ...
  methods: {
    ...
    edit(item) {
      this.$router.push(`/typeofpt/edit/${item.CodTipForPagto}/${item.id_base}`);
    },
    pagto(to) {
    ...
    search() {
      ...
      this.showWaitAboveAll();
      paymenttypeService.typelist(this.getLoggedId(), this.form.id_base, this.form.search, this.grids.default.currentPage, this.grids.default.perPage).then(
        response => {
    ...
    return {
        ...
        grids: {
            default: {
                ...
                items: [],
                fields: {
                    TipForPagto: { label: 'Nome', sortable: false },
                    ClassifPagtoSAP: { label: 'Classificação', sortable: false },
                    StaTipForPagto: { label: 'Ativo?', sortable: false },
                    actions: { label: 'Ações' }
                },
            }
        }
